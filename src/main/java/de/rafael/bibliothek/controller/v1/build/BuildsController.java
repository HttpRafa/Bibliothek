...
public class BuildsController {
    ...
    @Autowired
    public BuildsController(ProjectRepository projects, VersionRepository versions, BuildRepository builds) {
        this.projects = projects;
        this.versions = versions;
        this.builds = builds;
    }

    @GetMapping("/v1/projects/{project:" + Patterns.PROJECT_NAME + "}/versions/{version:" + Patterns.VERSION_NAME + "}/builds")
    public ResponseEntity<?> build(@PathVariable("project") @Pattern(regexp = Patterns.PROJECT_NAME) String projectName, @PathVariable("version") @Pattern(regexp = Patterns.VERSION_NAME) String versionName) {
        ...
        var builds = this.builds.findAllByProjectAndVersion(project._id(), version._id()).stream().map(build -> new ResponseBuild(build.number(), build.timestamp(), build.channel(), build.displayMode(), build.changes(), build.downloads())).toList();
        return ResponseEntity.ok().cacheControl(CACHE).body(new Response(project.name(), project.friendlyName(), version.name(), builds));
    }
